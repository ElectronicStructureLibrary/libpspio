...
/** 
 * @file abinit_fhi.c
 * @brief Utility routines to read/write Abinit files using the FHI
 *        format.
 */
#include <stdio.h>
#include <assert.h>
...
int abinit_fhi_read(FILE *fp, pspio_pspdata_t **pspdata){
  ...
  CHECK_ERROR(sscanf(line, "%lf %d", &zval, &(*pspdata)->n_potentials ) == 2, PSPIO_EFILE_CORRUPT);
  ...
  // Check internal consistency of the file
  CHECK_ERROR(abs(zval-(*pspdata)->zvalence) < 1.0e-14, PSPIO_EFILE_CORRUPT);
  CHECK_ERROR((*pspdata)->n_potentials == (*pspdata)->l_max + 1, PSPIO_EFILE_CORRUPT);
  ...
  for (l=0; l < (*pspdata)->l_max+1; l++) {
    CHECK_ERROR(fgets(line, PSPIO_STRLEN_LINE, fp) != NULL, PSPIO_EIO);
    CHECK_ERROR(sscanf(line, "%d %lf", &np, &r12 ) == 2, PSPIO_EFILE_CORRUPT);
    ...
    for (ir=0; ir<np; ir++) {
      CHECK_ERROR(fgets(line, PSPIO_STRLEN_LINE, fp) != NULL, PSPIO_EIO);
      CHECK_ERROR(sscanf(line, "%d %lf %lf %lf", &i, &r[ir], &wf[ir], &v[ir]) == 4, PSPIO_EFILE_CORRUPT);
      wf[ir] = wf[ir]/r[ir];
    }

    if (l==0) {
      // We will use the first block to generate the mesh, as the mesh
      // is supposed to be the same for all blocks (at least that is
  ...
  // Non-linear core-corrections (as determined from the Abinit header)
  ...
  if (have_nlcc) {
	...
	CHECK_ERROR(fgets(line, PSPIO_STRLEN_LINE, fp) != NULL, PSPIO_EIO);
      ...
      CHECK_ERROR(sscanf(line, "%lf %lf %lf %lf", &r12, &cd[ir], &cdp[ir], &cdpp[ir]) == 4, PSPIO_EFILE_CORRUPT);
